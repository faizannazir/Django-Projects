...
from .models import Employee, Attendance, User, Leave
from .forms import EmployeeRegistrationForm, EmployeeLoginForm, ApplyLeave
...
def home(request):
    if request.user.is_staff or request.user.is_superuser:
        ...
        attendances = get_list_or_404(Attendance.objects.filter(
            employee=employee).order_by('-date'))
        # attendances = get_list_or_404(Attendance,employee=employee)
        # Attendance.objects.filter(employee=employee).order_by('-date')
